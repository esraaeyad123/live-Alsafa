                    ...
                    <div class="card-body p-0">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                @foreach ($subcategories as $su)
                                    <tr>
                                        <td>{{ $su->id }}</td>

                                        <td>{{ $su->name }}</td>
                                        <td>{{ $su->category->name }}</td>


                                        <td class="d-flex">
                                            <a href="{{ route('subcategories.edit',$su->id)}}" class="btn btn-sm btn-primary mr-1">

                                                <i class="fas fa-edit"></i> </a>



                                            <form action="{{ route('subcategories.destroy',$su->id)}}" class="mr-1" method="POST">
                                                @method('DELETE')
                                                @csrf 
